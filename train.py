...
# os.environ['WANDB_MODE'] = 'dryrun' # Disable wandb synchronization
PROJECT = "AttentionDepth"
logging = True
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ###################################### Load model ##############################################
    model = models.AttentionDepth(n_bins=args.n_bins, min_val=args.min_depth, max_val=args.max_depth,
...
def train(model, args, epochs=10, experiment_name="Depth", lr=0.0001, root=".", device=None,
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, lr, epochs=epochs, steps_per_epoch=len(train_loader),
                                              cycle_momentum=True,pct_start=0.5,
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            square_mask = cv2.imread(args.square_mask, cv2.IMREAD_UNCHANGED) 
            square_mask = square_mask[:,:,0]
            new_mask = np.logical_and(mask.cpu().numpy(), square_mask) 
            new_mask = torch.tensor(new_mask.astype(np.float32))
            l_dense = criterion_ueff(pred, depth, mask=new_mask.to(torch.bool), interpolate=True) 
...
def validate(args, model, test_loader, criterion_ueff, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(
            ...
            square_mask = cv2.imread(args.square_mask, cv2.IMREAD_UNCHANGED) 
            square_mask = square_mask[:,:,0]
            new_mask = np.logical_and(mask.cpu().numpy(), square_mask) # mask logical AND mask square_mask to validate
            new_mask = torch.tensor(new_mask.astype(np.float32))
            l_dense = criterion_ueff(pred, depth, mask=new_mask.to(torch.bool), interpolate=True)
            ...
            if args.garg_crop or args.eigen_crop or args.square_crop:
                ...
                elif args.square_crop:
                    eval_mask = square_mask
            else :
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', type=str,   help='path of pretrained encoder', default="/home/lc/code/pretrain/swin_tiny_patch4_window7_224_22k.pth")
    ...
    parser.add_argument('--div-factor', '--div_factor', default=3.3, type=float, help="Initial div factor for lr")
    ...
    parser.add_argument("--name", default="AttentionDepth")
    ...
    parser.add_argument("--dataset", default='burden', type=str, help="Dataset to train on")

    parser.add_argument("--data_path", default='/home/lc/dataset_burden/img_rgb_fire', type=str,
                        help="path to dataset")
    parser.add_argument("--gt_path", default='/home/lc/dataset_burden/groundtruth', type=str,
                        help="path to dataset")
    parser.add_argument('--filenames_file',
                        default="./train_test_inputs/burden_train_files.txt",
                        type=str, help='path to the filenames text file')
 
    parser.add_argument('--input_height', type=int, help='input height', default=285)
    parser.add_argument('--input_width', type=int, help='input width', default=349)
    parser.add_argument('--max_depth', type=float, help='maximum depth in estimation', default=16)
    ...
    parser.add_argument('--data_path_eval',
                        default="/home/lc/dataset_burden/img_rgb_fire",
                        type=str, help='path to the data for online evaluation')
    parser.add_argument('--gt_path_eval', default="/home/lc/dataset_burden/groundtruth",
                        type=str, help='path to the groundtruth data for online evaluation')
    parser.add_argument('--filenames_file_eval',
                        default="./train_test_inputs/burden_eval_files.txt",
                        type=str, help='path to the filenames text file for online evaluation')
    parser.add_argument('--min_depth_eval', type=float, help='minimum depth for evaluation', default=1e-3)
    parser.add_argument('--max_depth_eval', type=float, help='maximum depth for evaluation', default=16)
    parser.add_argument('--eigen_crop', default=False, help='if set, crops according to Eigen NIPS14',
                        action='store_true')
    parser.add_argument('--garg_crop', help='if set, crops according to Garg  ECCV16', action='store_true')
    parser.add_argument('--square_mask', type=str, help='path to the square mask', default="./mask.png")
    parser.add_argument('--square_crop', type=str, help='choose if load the square mask', default=False)
